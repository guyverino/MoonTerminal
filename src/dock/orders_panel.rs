//! Нижний док: таблица открытых ордеров группы (на всю ширину окна).
//! Колонки: Ядро · Сторона · Токен · Size · SL · TS · VStop · Buy · Цена · Fill · Strat.

use crate::feed::OrderRow;
use crate::shell::theme;

pub fn show(ctx: &egui::Context, rows: &[(String, OrderRow)]) {
    egui::TopBottomPanel::bottom("orders")
        .resizable(true)
        ...
        .show(ctx, |ui| {
            ui.add_space(4.0);
            ui.label(
                egui::RichText::new(t!("orders.title", count = rows.len())).weak(),
            );
            ui.add_space(2.0);
