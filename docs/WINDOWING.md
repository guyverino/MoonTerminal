...
- `tool_window_options` - tool окна вроде настроек и стратегий.
- `detached_window_options` - открепленные панели и chart windows.
...
custom titlebar через `MoonWindowFrame::detached_panel(...)`; иначе получаем
четвертый визуальный тип окна, которого нет в дизайне.

## Owner передается только там, где есть Window
...
- live UI click: взять `window.window_handle()` в callback, где есть `Window`;
  дальше передать `Some(owner)`.
- restore/startup: передать `None`.

Например detached chart:

- double click по вкладке вызывает detach с `Some(window.window_handle())`;
- восстановление из `charts.json` открывает окно с `owner = None`.
