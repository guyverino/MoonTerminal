...
impl Dock {
    ...
    pub fn show(
        ...
        // Левая панель ордера — часть контейнера чарта: при закрытом чарте скрыта
        // (контейнер пустой/серый, центральная область не растягивается контентом).
        if chart_open {
            if let Some(action) = order::show(ctx, &self.controls) {
                log::info!(
                    "[ui] order action {:?}, size={} (торговля пока не подключена)",
                    action,
                    self.controls.active_size()
                );
            }
        }
