...
/// Запас канваса справа от экрана как ДОЛЯ ширины зоны графика: сколько можно
/// проскроллить вперёд UV-сдвигом до полного re-bake. Маленький (20%) намеренно —
/// «ручники» часто скроллят/двигают вид, что и так инвалидирует канвас (full
/// rebake), поэтому большой запас лишь жрёт память без выигрыша (отзыв ядра-разраба).
pub const MARGIN_FRAC: f32 = 0.20;

/// Минимальный запас в пикселях (на узких окнах 20% дало бы слишком мало).
pub const MARGIN_MIN_PX: u32 = 128;

/// Запас канваса для заданной ширины зоны графика (физ. px).
pub fn margin_px(chart_w: f32) -> u32 {
    ((chart_w * MARGIN_FRAC) as u32).max(MARGIN_MIN_PX)
}
