...
// Подписи осей (axes) рисуются egui → остались в egui-оболочке (moon-terminal).
// Здесь — только layout-константы ширины/высоты осей (нужны движку и axes).
...
// Ре-экспортим под прежним путём `moon_core::data`, чтобы рендер не править.
...
use moon_core::market::MarketView;
...
pub struct Chart {
    ...
    glass_scratch: Vec<moon_core::data::LevelInstance>,
    last_glass_lo: f32,
    last_glass_hi: f32,
}
...
impl Chart {
    ...
    pub fn render(
        ...
        let time_axis_h = TIME_AXIS_H * ppp;
