...
pub struct DetectRow {
    ...
    /// AddToChart=Yes у стратегии — детект авто-добавляет график монеты в общий
    /// AddToChart-контейнер (вкладку над чартом).
    pub add_to_chart: bool,
    /// KeepInChart, сек — сколько держать авто-график монеты в контейнере, прежде
    /// чем закрыть (контейнер/вкладка остаётся). Дефолт 60.
