...
Словари **ещё не подключены** к рантайму терминала: в коде строки захардкожены, вызовов
`t!` нет. Чтобы словари заработали, на этапе подключения нужно:
1. добавить зависимость `rust-i18n` в крейт `moon-ui-gpui`;
2. вызвать `i18n!("../../locales", fallback = "en")` (путь относительно манифеста крейта);
   сейчас `i18n!` есть только внутри MoonUI и смотрит на словари самого MoonUI.
